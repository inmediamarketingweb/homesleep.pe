...
function Filtros({ productos, setProductosFiltrados, filtersActive, onClose }){
    ...
    const filtrarProductos = useCallback(() => {
        ...
        let productosFiltrados = productos.filter((producto) => {
            ...
            const cumpleFiltros = Object.keys(filtrosSeleccionados).every((categoriaFiltro) => {
                const opcionesSeleccionadas = Array.from(filtrosSeleccionados[categoriaFiltro]);
                
                if (categoriaFiltro === 'marca') {
                    const marcaProducto = producto.marca ? normalizarTexto(producto.marca) : '';
                    return opcionesSeleccionadas.some(marcaSeleccionada => 
                        normalizarTexto(marcaSeleccionada) === marcaProducto
                    );
                } else {
                    // Para otros filtros, buscar en detalles del producto
                    if (producto["detalles-del-producto"]) {
                        return producto["detalles-del-producto"].some((detalle) => {
                            const valorDetalle = detalle[categoriaFiltro];
                            if (!valorDetalle) return false;
                            return opcionesSeleccionadas.some(opcion => 
                                normalizarTexto(opcion) === normalizarTexto(valorDetalle)
                            );
                        });
                    }
                    return false;
            ...
            if (rangoDePrecioSeleccionado) {
                const rango = rangosDePrecio.find(r => r.id === rangoDePrecioSeleccionado);
                if (rango) {
                    const precioProducto = producto.precioVenta || producto.precioNormal || 0;
                    cumpleRangoPrecio = precioProducto >= rango.min && 
                                       precioProducto <= (rango.max === Infinity ? Number.MAX_SAFE_INTEGER : rango.max);
                }
            }

            // Verificar envío gratis
            const cumpleEnvioGratis = !envioGratisSeleccionado || 
                                    producto["tipo-de-envio"]?.toLowerCase() === "gratis";

            return cumpleFiltros && cumpleRangoPrecio && cumpleEnvioGratis;
        });

        setProductosFiltrados(productosFiltrados);
    }, [productos, filtrosSeleccionados, rangoDePrecioSeleccionado, envioGratisSeleccionado, rangosDePrecio, setProductosFiltrados]);
    ...
    useEffect(() => {
        ...
        setSearchParams(params);
    }, [filtrosSeleccionados, rangoDePrecioSeleccionado, envioGratisSeleccionado, setSearchParams]);
            ...
            <div className={`filters-container ${filtersActive ? "active" : ""}`}>
                <div className="filters-content">
                            ...
                            {rangosDePrecio.map((rango) => (
                                <li key={rango.id}>
                                    <input 
                                        ...
                                        className="radio-input"
                                    />
                                    <label htmlFor={rango.id} className="radio-label">{rango.titulo}</label>
                                </li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div className="filtro-envio-gratis">
                        ...
                        <button 
                            type="button" 
                            onClick={toggleEnvioGratisFilter} 
                            className={envioGratisSeleccionado ? "active" : ""} 
                            aria-pressed={envioGratisSeleccionado}
                        >
                            <span></span>
                        </button>
                    </div>
                    ...
                    {filtros.map((filtro) => (
                        <div className="filter d-flex-column gap-10" key={filtro.nombre}>
                            <div 
                                className={`filter-title ${activeFilter === filtro.nombre ? "active" : ""}`} 
                                onClick={() => handleToggleFilter(filtro.nombre)} 
                                role="button" 
                                tabIndex="0" 
                                aria-expanded={activeFilter === filtro.nombre}
                            >
                                <h3 className="title">{filtro.titulo}:</h3>
                                <span className="material-icons">keyboard_arrow_down</span>
                            </div>
                            ...
                            <ul className={`${activeFilter === filtro.nombre ? "active" : ""}`}>
                                {Array.isArray(filtro.lista) ? (
                                    <>
                                        ...
                                        <li 
                                            key={`${filtro.nombre}-todos`} 
                                            className={!filtrosSeleccionados[filtro.nombre] ? "active" : ""} 
                                            onClick={() => handleVerTodos(filtro.nombre)} 
                                            role="button" 
                                            tabIndex="0"
                                        >
                                            <input type="radio" readOnly checked={!filtrosSeleccionados[filtro.nombre]}/>
